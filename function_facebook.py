            ...
            for i in range(1):
                ...
                while cont:
                    try:
                        ...
                        url_frase = f"https://www.papiron.com.br/api/frases/random/?faculdade={faculdade.upper()}&midia=facebook"
                        req_frase = requests.get(url=url_frase)
                        data_frase = req_frase.json()

                        
                        codigo = data["codigo"]
                        if data_frase["midia"] == "whatsapp":
                            url_grupo =  f"https://www.papiron.com.br/grupo_whatsapp/{codigo or ''}"
                            frase_selecionada=f"GRUPO DE ESTUDO\n{data_frase['texto']}\n{data_frase['url']}"

                        elif data_frase["midia"] == "facebook":
                            frase_selecionada = f"{data_frase['texto']}\n{data_frase['url']}" if data_frase['url'] else data_frase['texto']

                        print(f"   >> URL Grupo: {url_grupo_fb}")
